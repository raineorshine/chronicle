...
Treat `" - "` (space-hyphen-space), `" | "` (space-pipe-space), and `" / "`
(space-slash-space) as the subtask separators. A title is split on the leftmost
occurrence of any of them.

Do not split ordinary hyphenated words or slash-joined paths like `a/b`.
